...
Flags:
  ...
  -T, --auto-test-tag string         (optional) Automation test tag(separated by space)
      ...
      --cdfurl string                (optional) download url of CDF
      --controllerimgtags string     (optional) one service, one controller image tag(separated by space), using stable by default
  -c, --deepclean string             (optional) revert vm to snapshot, revert vm in ShangHai --> true or default,in Sunnyvale --> sunny. (default "false")
  ...
  -d, --depth int                    (optional) depth -d 1 or -d 3 (default 3)
      ...
      --external-db-json string      (optional) external db json file
      --external-ldap-json string    (optional) external ldap json file
  ...
  -m, --masterip string              (required) IP address of the master node
      --mix-mode string              (optional) true or false , if true will apply external SM to suite.  (default "false")
  ...
  -s, --service string               (optional) Itsma services to install(separated by space), all services will be installed by default
      --suite-data-tag string        (optional) tag of suite-data (default "latest")
      --suite-size string            (optional)  Itom Suite Size optional choice 'demo' ,'xsmall', 'small', 'medium', 'large' default demo
      --suite-version string         (optional) suite version ,only support 2017.11.001 2018.02 2018.05.....  (default "2018.05")
      ...
      --upgrade-image-tag string     (optional) The docker image tag of itsma suite update (itsma/itom-itsma-update) 
...
```bash
  ...
  11. Use Thinpool for the Docker devicemapper storage driver(disable Thinpool as default)
   ./suitectl deploy -m masterip -n "node1_ip node2_ip node3_ip" --thinpool "true"
  ...
  15. Upgrade suite //not support in 3.0.0.1
  # CDF version >= 147 , kit version >= 1.0.0.10
    15.1 Prepare suite env
    15.2 ./suitectl deploy -m masterip -n "node1_ip node2_ip node3_ip" --upgrade true 
  16. Specify the password of mng-protal //deleted since 3.0.0.1
  ...
  20. Deploy specified suite version
  # kit version >= 4.0.0.1 , only support deploy suite 2017.11.001 2018.02 2018.05 
  ./suitectl deploy -m masterip -n "node1_ip node2_ip node3_ip" --suite-version "2017.11.001" 
  21. Deploy CDF and SMA on single node
  # kit version >=4.0.0.5
  ./suitectl deploy -m masterip -n masterip
  22. Revert VM in Sunnyvale
  ./suitectl deploy -m masterip -n "node1_ip node2_ip node3_ip" -c sunny 
  23. Deploy suite with extarnal LDAP
  23.1 Deploy H Full NG with external ldap and db
  ./suitectl deploy -m masterip -n "node1_ip node2_ip node3_ip"  --external-db-json external-db-ldap-json-example/H-full.json --external-ldap-json external-db-ldap-json-example/ldap.json
  23.2 Deploy H-p1  with external ldap and db
  ./suitectl deploy -m masterip -n "node1_ip node2_ip node3_ip"  --external-db-json external-db-ldap-json-example/H-p1.json --external-ldap-json external-db-ldap-json-example/ldap.json --ds "itom-sm itom-cmdb"
  23.3 Deploy X-Full with external ldap and db
  ./suitectl deploy -m masterip -n "node1_ip node2_ip node3_ip"  --external-db-json external-db-ldap-json-example/X-full.json --external-ldap-json external-db-ldap-json-example/ldap.json -M "X_MODE" 
  for more details check json template in external-db-ldap-json-example.
  NOTE: only SM and UCMDB support both Oracle and Postgres
```
## Deploy Release ENV
refer to [how_to_deploy_release_env][2]

| SMA Version        | CDF Version   |  Kit Version  | Required  Parameters|
| -------- | :----:  | :----:  | ------- |
| 2017.11(GA)     | 2017.06.00158 |  v2.0.0.1     | -t 171118  --suite-data-tag  2017.11.018 |
| 2017.11.001     | 2017.10   | v3.0.0.3     |  -t "hotfix-2017.11" |
|2017.11.001    |  2018.01| NOT Supported | |
|2018.02  |  2017.10      | v3.0.0.2            | --suite-data-tag "171232" |
| 2018.02(Release) |   2018.01     |  v4.0.0.9  | -t 180215 --suite-version "2018.02"  |
| 2018.05 |   2018.01     |  v4.0.0.9  |  --suite-version "2018.05"  |
| 2018.02 |   2018.05     |  v5.0.0.x  |  --suite-version "2018.02"  |
| 2018.05 |   2018.05     |  v5.0.0.x |  --suite-version "2018.05"  |
...
### v5.0.0.6
```bash
update CDF to 2018.05.0096
add 2 db volumes for pgha
```

### v5.0.0.5
```bash
update CDF to 2018.0500087
```

### v5.0.0.4
```bash
upgrade CDF to 2018.05.0052
```
### v5.0.0.3
```bash
update CDF to 2018.05.0047
support upgrade SMA from 2018.02 to 2018.05 
```

### v5.0.0.2
```bash
support external DB and LDAP(example 23)
      --external-db-json string      (optional) external db json file
      --external-ldap-json string    (optional) external ldap json file
...
### v5.0.0.1
```bash
Update CDF to 2018.04.0036
Deploy SMA 2018.05 as default(use --suite-version to specified other suite version,example 20)
Don't support complex upgrade

```

### v4.0.0.9
```bash
Deploy SMA 2018.05 as default
Support SMA complex upgrade from 2018.02 to 2018.05
```
### v4.0.0.8
```bash
update CDF to 2018.01.00118 MR2
```

### v4.0.0.7 
```bash
update CDF to 2018.01.00112
```

### v4.0.0.6
```bash
update CDF to 2018.01.0098
support revert vm in Sunnyveal(example 22)
  22. Revert VM in Sunnyvale
  ./suitectl deploy -m masterip -n "node1_ip node2_ip node3_ip" -c sunny 
...
### v4.0.0.5
```bash
support deploy SMA adn CDF on single node.(example 21)
add 3 pv
	rabbitmq-infra-rabbitmq-0   Bound     itsma1-rabbitmq-infra-rabbitmq-0   5Gi        RWX
	rabbitmq-infra-rabbitmq-1   Bound     itsma1-rabbitmq-infra-rabbitmq-1   5Gi        RWX
	rabbitmq-infra-rabbitmq-2   Bound     itsma1-rabbitmq-infra-rabbitmq-2   5Gi        RWX

```

### v4.0.0.4
```bash
fix unable to reinstall from UI bug

```
### v4.0.0.3
```bash
update CDF  to 2018.01.0081
fix thinpool bug
update mng-portal password to 1Qaz2wsx,
update regular expression for suite-data new json format
...
```bash
add 3 pv
	rabbitmq-pro-rabbitmq-0
	rabbitmq-pro-rabbitmq-1
	rabbitmq-pro-rabbitmq-2
```

### v4.0.0.1
```bash
update CDF to 2018.01.0051
support new vm template
add arg --suite-version 
  Example:
    20. Deploy specified suite version
      # kit version >= 4.0.0.1 , only support deploy suite 2017.11.001 2018.02 
      ./suitectl deploy -m masterip -n "node1_ip node2_ip node3_ip" --suite-version "2017.11.001" 
use latest as default suite-data image tag
```


### v3.0.0.2
```bash
update CDF to 2017.10.00301
support complex update
support thinpool
support SMA 2017.11 installation
...
update CDF to Oct release RC3
delete arg --admin-password ,CDF mng-portal use "1Qaz2wsx" as default password
update suite-data
do not support suite update(fix in next version)
```


### v2.0.0.1
```bash
update vCenter password and address
```
### v1.0.0.23
```bash
...
```
  ...
  [2]: https://github.houston.softwaregrp.net/SMA-RnD/suite-kit/blob/master/deploy_release_env.md
