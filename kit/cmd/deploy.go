...
var (
	...
	MixMode				string
	SuiteVersion		string
	ExternalLdapJsonPath	string
	ExternalDBJsonPath	string
	ExternalLdapJsonContent		string
	ExternalDBJsonContent		string
...
var deployCmd = &cobra.Command{
	...
	Examples:
	  ...
	  # kit version >= 4.0.0.2 , only support deploy suite 2017.11.001 2018.02 2018.05 
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		deploy.SuiteVersion = SuiteVersion
		deploy.ExternalLdapJsonPath = ExternalLdapJsonPath
		deploy.ExternalDBJsonPath = ExternalDBJsonPath
		//get json content for ansible
		if ExternalLdapJsonPath != ""{
			// external ldap json check
			External_Ldap_Main()
			deploy.ExternalLdapJsonContent = string(JsonFileCheck(ExternalLdapJsonPath))
		}
		if ExternalDBJsonPath != ""{
			// external db json check
			External_DB_Main()
			deploy.ExternalDBJsonContent = string(JsonFileCheck(ExternalDBJsonPath))
		}

		//fmt.Println(deploy.ExternalDBJsonContent)
...
func checkFlags() error {
	...
	if CdfUrl == ""{
		//uncomment this line after mvn install cdf zip file to right nexus folder.
		//CdfUrl = GetSuitePlatformLatestDownloadUrl()
		CdfUrl = "http://shc-nexus-repo.hpeswlab.net:8080/repository/itsma-releases/com/hpe/shared/services/cdf/2018.05.0096/cdf-2018.05.0096.zip"
...
func init() {
	...
	deployCmd.Flags().StringVarP(&DeepClean, "deepclean", "c", "false", "(optional) revert vm to snapshot, revert vm in ShangHai --> true or default,in Sunnyvale --> sunny.")
	...
	deployCmd.Flags().StringVarP(&SuiteDataImageTag, "suite-data-tag", "", "latest", "(optional) tag of suite-data")
	deployCmd.Flags().StringVarP(&MixMode, "mix-mode", "", "false", "(optional) true or false , if true will apply external SM to suite. ")
	deployCmd.Flags().StringVarP(&SuiteVersion, "suite-version", "", "2018.05", "(optional) suite version ,only support 2017.11.001 2018.02 2018.05..... ")
	deployCmd.Flags().StringVarP(&ExternalLdapJsonPath, "external-ldap-json", "", "", "(optional) external ldap json file")
	deployCmd.Flags().StringVarP(&ExternalDBJsonPath, "external-db-json", "", "", "(optional) external db json file")
